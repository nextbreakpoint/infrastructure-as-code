...
docker run --rm -it --net=services \
  ...
  -v $(pwd)/secrets/password_keystore.txt:/secrets/password_keystore.txt \
  -v $(pwd)/secrets/password_truststore.txt:/secrets/password_truststore.txt \
  -v $(pwd)/secrets/client-ssl.properties:/secrets/client-ssl.properties \
  $KAFKA_IMAGE \

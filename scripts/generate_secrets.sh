...
cat <<EOF > $OUTPUT/cliet-ssl.properites
...
cat <<EOF > $OUTPUT/password_keystore.txt
${PASSWORD}
EOF

cat <<EOF > $OUTPUT/password_truststore.txt
${PASSWORD}
EOF

cat <<EOF > $OUTPUT/kafka_jaas.conf
Client {
...
cat <<EOF > $OUTPUT/zookeeper_jaas.conf
QuorumServer {
       org.apache.zookeeper.server.auth.DigestLoginModule required
...
cp $OUTPUT/truststore-client.jks $DST
cp $OUTPUT/truststore-server.jks $DST
cp $OUTPUT/password_keystore.txt $DST
cp $OUTPUT/password_truststore.txt $DST
cp $OUTPUT/kafka_jaas.conf $DST/client_jaas.conf

DST=$ROOT/secrets/environments/$ENVIRONMENT/zookeeper

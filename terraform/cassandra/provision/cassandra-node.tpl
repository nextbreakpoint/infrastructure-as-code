...
runcmd:
  ...
  - sudo -u ubuntu docker run -d --name=cassandra --restart unless-stopped --net=host -p 7000:7000 -e CASSANDRA_BROADCAST_ADDRESS=$HOST_IP_ADDRESS -e CASSANDRA_RPC_ADDRESS=$HOST_IP_ADDRESS -e CASSANDRA_LISTEN_ADDRESS=$HOST_IP_ADDRESS -e CASSANDRA_RACK=RACK1 -e CASSANDRA_DC=DC1 -e CASSANDRA_SEEDS=${cassandra_nodes} -v /cassandra/data:/var/lib/cassandra -v /cassandra/logs:/var/log/cassandra cassandra:latest
  - sudo -u ubuntu docker build -t filebeat:${filebeat_version} /filebeat/docker
  - sudo -u ubuntu docker run -d --name=filebeat --restart unless-stopped --net=host --log-driver json-file --log-driver json-file -v /filebeat/config/filebeat.yml:/usr/share/filebeat/filebeat.yml -v /filebeat/config/secrets:/filebeat/config/secrets -v /var/log/syslog:/var/log/syslog filebeat:${filebeat_version}

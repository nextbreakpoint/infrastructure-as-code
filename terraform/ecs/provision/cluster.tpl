...
runcmd:
  ...
  - sudo -u docker docker build -t filebeat:${filebeat_version} /filebeat/docker
  - sudo -u docker docker run -d --name=filebeat --restart unless-stopped --user=root:root --net=host --log-driver json-file -v /filebeat/config/filebeat.yml:/usr/share/filebeat/filebeat.yml -v /filebeat/config/secrets:/filebeat/config/secrets -v /var/lib/docker/containers:/var/log/docker/containers filebeat:${filebeat_version}
